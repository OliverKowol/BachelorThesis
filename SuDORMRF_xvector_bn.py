...
class SuDORMRF_xvector_bn(nn.Module):
    def __init__(self,
                 out_channels=256,
                 in_channels=512,
                 num_blocks=16,
                 ...
                 enc_num_basis=512,
                 num_sources=1):
        super(SuDORMRF_xvector_bn, self).__init__()
        ...
        self.encoder = nn.Conv1d(in_channels=1, out_channels=enc_num_basis,
                                 ...
                                 bias=False)
        self.bn_encoder = nn.BatchNorm1d(enc_num_basis)
        ...
        # create modified input channel size: 512 + 512 = 1024 instead of 512
        emb_shape = 512
        self.new_in_channel = emb_shape + enc_num_basis

        # Norm before the rest, and apply one more dense layer
        self.ln = GlobLN(self.new_in_channel)  # changed for x-vector
        # self.ln = GlobLN(self.enc_num_basis)

        self.bottleneck = nn.Conv1d(
            in_channels=self.new_in_channel,  # changed for x-vector
    ...
    # Forward pass
    def forward(self, input_wav, utterance):
        # Front end
        x = self.pad_to_appropriate_length(input_wav)

        x = self.encoder(x)
        x = self.bn_encoder(x)  # ADDED - Batch normalisation
        ...
        x-Vector
        """
        embedding = self.xvector.encode_batch(utterance)
        embedding = embedding.cuda()

        embedding = torch.reshape(embedding, (embedding.shape[0], embedding.shape[2], embedding.shape[1]))
        m = torch.nn.Upsample((x.shape[2])).cuda()
        embedding_ = m(embedding)
        embedding_ = embedding_.cuda()
        x = torch.cat((x, embedding_), dim=1)
        x = x.cuda()
