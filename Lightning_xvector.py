...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        :return: both, tensor value and path of target, noise, mix and utterance
        """
        row = self.data.iloc[index]
        target_path = row['Target']
        noise_path = row['Noise']
        mix_path = row['Mix']
        utterance_path = row['Utterance']

        target, _ = torchaudio.load(target_path)
        noise, _ = torchaudio.load(noise_path)
        mix, _ = torchaudio.load(mix_path)
        utterance, _ = torchaudio.load(utterance_path)

        return target, noise, mix, utterance[0,:], target_path, noise_path, mix_path, utterance_path
...
class TSE_Model(pl.LightningModule):
    ...
    def forward(self, x, u):
        # for x-vector also pass utterance through model
    ...
    def validation_step(self, batch, batch_idx):
        ...
        target = target.cuda()
        mix = mix.cuda()
        utterance = utterance.cuda()  # also retrieve utterance for x-vector
        ...
        loss = scale_invariant_signal_distortion_ratio(ests, target).mean(axis=1)  
        loss = -torch.sum(loss) / len(ests)
        print("Epoch " + str(self.current_epoch) + ", validation batch " + str(batch_idx) + " loss: " + str(loss))
        ...
        self.log("validation_pesq", pesq, on_step=False, on_epoch=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        :param batch_idx: index of batch
        """
        target, _, mix, utterance, target_path, noise_path, mix_path, utterance_path = batch
        target = target.cuda()
        mix = mix.cuda()
        utterance = utterance.cuda()
        ...
        df_test = pd.DataFrame(
            columns=["Mix path", "Target path", "Noise path", "Estimate path", "Utterance path",
                     "si-sdr", "si-sdr-i", "sdr", "pesq"])
        ...
        for i in range(len(ests)):
            ...
            si_sdr_i = scale_invariant_signal_distortion_ratio(ests[i], target[i]) - scale_invariant_signal_distortion_ratio(mix[i], target[i])
            ...
            df_test.loc[i, "Utterance path"] = utterance_path[i]
    ...
    def val_dataloader(self):
        ...
        val_dl = DataLoader(val_data, batch_size=self.batch_size, shuffle=False,
                            num_workers=6)
        return val_dl
