...
from Lightning_xvector import TSE_Model
from SuDORMRF_xvector_bn import SuDORMRF_xvector_bn
...
if __name__ == '__main__':
    ...
    model = SuDORMRF_xvector_bn(out_channels=256,
                                in_channels=512,
                                num_blocks=16,
                                upsampling_depth=5,
                                enc_kernel_size=21,  # tried 41, but no improvement
                                enc_num_basis=512,
                                num_sources=1)
    ...
    not_init = ['mask_net.1.weight', 'mask_net.1.bias', 'decoder.weight', 'mask_nl_class',
                'ln.gamma', 'ln.beta', 'bottleneck.weight']  # xvector
    ...
    model = TSE_Model(model=model, train_num_rows=20000, val_num_rows=5000, test_num_rows=3000, batch_size=2)
    ...
    trainer = pl.Trainer(logger=logger,
                         ...
                         accelerator="gpu",
                         devices=2,
                         accumulate_grad_batches=2  # accumulate 2 batches because of batch_size=2
                         )
